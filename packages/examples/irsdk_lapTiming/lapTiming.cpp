...
*/
      ...
      if (curDistPct != -1) {
        ...
        if (g_lastDistPct[i] > 0.9f && curDistPct < 0.1f) {
          ...
          const double curLapStartTime =
              interpolateTimeAcrossPoint(g_lastTime, curTime, g_lastDistPct[i], curDistPct, 0);
    ...
    for (int i = 0; i < g_maxCars; i++) {
      if (linesUsed < maxLines) {
        ...
        if (g_CarIdxTrackSurface.getInt(i) != -1 || g_CarIdxLap.getInt(i) != -1 || g_CarIdxPosition.getInt(i) != 0) {
          printf(" %2d %3s %7.3f %2d %2d %2d %6.3f %2d %8.2f %5.2f %2d %2d %2d %2d %7.3f %7.3f %7.3f %7.3f %2d %d %2d "
                 "%2d %2d 0x%02x\n",
                 i, g_driverTableTable[i].carNumStr, g_CarIdxEstTime.getFloat(i), g_CarIdxGear.getInt(i),
                 g_CarIdxLap.getInt(i), g_CarIdxLapCompleted.getInt(i), g_CarIdxLapDistPct.getFloat(i),
                 g_CarIdxOnPitRoad.getBool(i), g_CarIdxRPM.getFloat(i), g_CarIdxSteer.getFloat(i),
                 g_CarIdxTrackSurface.getInt(i), g_CarIdxTrackSurfaceMaterial.getInt(i), g_CarIdxPosition.getInt(i),
                 g_CarIdxClassPosition.getInt(i), g_CarIdxF2Time.getFloat(i),
                 //****Note, don't use this one any more, it is replaced by CarIdxLastLapTime
                 g_lapTime[i],
                 // new variables, check if they exist on members
  ...
  void run() {
    ...
    //   if (LiveClient::GetInstance().wasSessionInfoUpdated()) {
    //     //processYAMLSessionString(LiveClient::GetInstance().getSessionInfoStr());
    //     wasUpdated = true;
    //   }
